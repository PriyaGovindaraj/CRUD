...
componentDidMount() {
      axios.get('/vehicletype/edit/'+this.props.match.params.id)
          .then(response => {
              this.setState({ 
  ...
  onSubmit(e) {
    e.preventDefault();
        ...
        vehicle_model:this.state.vehicle_model
         ...
         axios.post('/vehicletype/update/'+this.props.match.params.id, obj).then(res=>console.log(res.data)).then(() => this.setState({ redirect: true }));
